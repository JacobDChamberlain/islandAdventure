...
@export var snap_to_roof: bool = false     # sit on the BUILDING ROOF at this x/z (city only)
...
func _snap_to_ground() -> void:
	...
	if island == null:
		_base_y = position.y
		return
	if snap_to_roof and island.has_method("building_top_at"):
		global_position.y = island.building_top_at(global_position.x, global_position.z) + 1.4 + extra_height
	elif island.has_method("height_at"):
